...
const CORS_PROXY = 'https://corsproxy.io/?'; // Updated to a more reliable proxy
...
async function fetchBoards(attempt = 1, maxAttempts = 3) {
    try {
        ...
        console.error(`Error fetching boards (attempt ${attempt}):`, error);
        if (attempt < maxAttempts) {
            console.log(`Retrying fetchBoards (attempt ${attempt + 1})...`);
            return fetchBoards(attempt + 1, maxAttempts);
        }
        if (boardsList) {
            boardsList.innerHTML = `
                <div class="error-message">
                    Unable to load boards. Please check your connection or try again later.
                    <button id="retry-boards-btn">Retry</button>
                </div>`;
            const retryBtn = document.getElementById('retry-boards-btn');
            if (retryBtn) {
                retryBtn.addEventListener('click', () => loadBoards());
            }
        }
        return [];
    }
...
// Calculate image brightness for text color
async function getImageBrightness(imageUrl) {
    const cachedBrightness = localStorage.getItem(`brightness:${imageUrl}`);
    if (cachedBrightness) return parseFloat(cachedBrightness);
    ...
    return new Promise((resolve) => {
        ...
        img.onload = () => {
            try {
                ...
                localStorage.setItem(`brightness:${imageUrl}`, avgBrightness);
                resolve(avgBrightness);
            } catch (error) {
                console.error(`Error processing image ${imageUrl}:`, error);
                resolve(128); // Neutral brightness fallback
            }
        };
        img.onerror = () => {
...
async function displayThreads(threads, boardCode) {
    ...
    for (const thread of threads) {
        ...
        if (thread.tim && thread.ext) {
            const imageUrl = `${CORS_PROXY}${encodeURIComponent(`${IMAGE_BASE}${boardCode}/${thread.tim}${thread.ext}`)}`;
            threadItem.classList.add('has-image');
            threadItem.style.backgroundImage = `url(${imageUrl})`;

            // Attempt to load image and calculate brightness
            try {
                const brightness = await getImageBrightness(imageUrl);
                threadItem.classList.add(brightness > 128 ? 'dark-text' : 'light-text');
                imageLoaded = true;
            } catch (error) {
                console.error(`Failed to load or process image ${imageUrl}:`, error);
                threadItem.classList.add('dark-text'); // Fallback
                threadItem.style.backgroundImage = 'none'; // Remove broken image
            }
        } else {
            threadItem.classList.add('dark-text');
        }
...
function appendMessageWithReplies(boardCode, post, postMap) {
        ...
        if (replyMatch) {
            ...
            if (referencedPost) {
                ...
                if (referencedPost.tim && referencedPost.ext) {
                    const previewImgUrl = `${CORS_PROXY}${encodeURIComponent(`${IMAGE_BASE}${boardCode}/${referencedPost.tim}${referencedPost.ext}`)}`;
    ...
    if (post.tim && post.ext) {
        const imgUrl = `${CORS_PROXY}${encodeURIComponent(`${IMAGE_BASE}${boardCode}/${post.tim}${post.ext}`)}`;
        html += `
