...
const baseApiUrl = 'https://a.4cdn.org'; // 4chan API base URL
const baseImageUrl = 'https://i.4cdn.org'; // 4chan image base URL
...
async function loadThreads() {
    ...
    debugButton.onclick = () => {
        const debugInfo = `
            URL: ${baseApiUrl}/${board}/catalog.json\n
    ...
    try {
        const data = await fetchWithRetry(`${baseApiUrl}/${board}/catalog.json`);
...
async function loadThreadDetails(board, threadNo) {
    ...
    debugButton.onclick = () => {
        const debugInfo = `
            URL: ${baseApiUrl}/${board}/thread/${threadNo}.json\n
            Primary Proxy: ${corsProxy}\n
            Fallback Proxy: ${fallbackProxy}\n
    ...
    try {
        const threadData = await fetchWithRetry(`${baseApiUrl}/${board}/thread/${threadNo}.json`);
